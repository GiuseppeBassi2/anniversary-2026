...
export const story = [
  // ───────────────────────── ACT I ─────────────────────────
  a1.text({ slug: "one-year", title: "One year.", tone: "big" }),
  a1.text({ slug: "date", lines: ["September 26, 2025."] }),
  a1.media([photo("A", 36)], { slug: "opening", lines: ["And somehow all of this happened."] }),

  // TORONTO (A)
  a1.text({ slug: "toronto", kicker: "Chapter one", title: "Toronto.", lines: ["Where it started."] }),
  a1.media([photo("A", 1), photo("A", 17)], { lines: ["Two people who had no idea what they were getting into."] }),
  a1.media([photo("A", 3)], { lines: ["Min, looking at the skyline.", "Me, looking at Min."] }),
  a1.video(video("A", 10)),
  a1.media([photo("A", 5), photo("A", 4), photo("A", 6)], { lines: ["Priority one: food.", "Priority two: also food."] }),
  a1.video(video("A", 16)),
  a1.media([photo("A", 21)], { lines: ["Niagara.", "Even the weather tried to be romantic."] }),
  a1.media([photo("A", 20)], { lines: ["Same day. Same windswept hair."] }),
  a1.media([photo("A", 13), photo("A", 14), photo("A", 15), photo("A", 8)], { lines: ["Everyday things that somehow became favorites."] }),
  a1.video(video("A", 22)),
  a1.video(video("A", 23)),
  a1.media([photo("A", 29)], { lines: ["Autumn arrived. We got cozy."] }),
  a1.media([photo("A", 37), photo("A", 38)], { lines: ["Toronto in the fall is unfairly good."] }),
  a1.video(video("A", 39)),
  a1.video(video("A", 40)),
  a1.media([photo("A", 43), photo("A", 45), photo("A", 42)], { lines: ["Cinnamon rolls, fish, and Min claiming furniture."] }),
  a1.media([photo("A", 50)], { lines: ["Us, in a very loud arena."] }),
  a1.video(video("A", 52)),
  a1.media([photo("A", 46), photo("A", 53), photo("A", 54)], { lines: ["Flowers.", "Min. Obviously."] }),
  a1.video(video("A", 47)),
  a1.media(
    [photo("A", 28), photo("A", 35), photo("A", 48), photo("A", 49), photo("A", 55), photo("A", 60), photo("A", 12), photo("A", 19), photo("A", 25)],
    { layout: "dump", lines: ["Memory dump."] },
  ),
  a1.video(video("A", 56)),
  a1.media([photo("A", 57)], { lines: ["Still the best place to be."] }),
  a1.video(video("A", 58)),
  a1.media([video("A", 62), photo("A", 65)], { lines: ["Kitchen experiments."] }),
  a1.video(video("A", 59)),
  a1.media([photo("A", 63)], { lines: ["Matching sweatshirts.", "Zero regrets."] }),
